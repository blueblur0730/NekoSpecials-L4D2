void NekoAdminMenu_CreateNatives()
{
...
any NekoAdminMenu_REPlHandle(Handle plugin, int numParams)
{
	return GetMyHandle();
}

void NekoAdminMenu_OnPluginStart()
{
	if (LibraryExists("adminmenu") && ((top_menu = GetAdminTopMenu()) != null))
		OnAdminMenuReady(top_menu);
}

void NekoAdminMenu_OnLibraryRemoved(const char[] name)
{
	if (StrEqual(name, "adminmenu", false))
		top_menu = null;
}

void NekoAdminMenu_OnAdminMenuReady(Handle aTopMenu)
{
...
static void CategoryHandler(TopMenu topmenu, TopMenuAction action, TopMenuObject object_id, int client, char[] buffer, int maxlength)
{
...
static void AdminMenu_Neko(TopMenu topmenu, TopMenuAction action, TopMenuObject object_id, int client, char[] buffer, int maxlength)
{
	if (action == TopMenuAction_DisplayOption)
	{
		if (object_id == specials_menu)
		{
			if (SPECIALS_AVAILABLE_ADMINMENU())
			{
